...
void requestaddStatsToBuf(char* buf , Stats *stats, int is_double_space){
    /*
...
void requestError(int fd, char *cause, char *errnum, char *shortmsg, char *longmsg, Stats *stats)
{
    ...
    requestaddStatsToBuf(buf, stats, 1);
...
void requestServeDynamic(int fd, char *filename, char *cgiargs, Stats* stats)
{
    ...
    requestaddStatsToBuf(buf, stats, 0);
...
//        printf("-----thread %d is before servedynamic----\n", stats->stat_thread.thread_id);
        requestServeDynamic(fd, filename, cgiargs, stats);
//        printf("-----thread %d is after servedynamic----\n", stats->stat_thread.thread_id);
